...
* **Host Integration:** Docker writes to host volumes as root. This breaks file permissions. We require the container user to map directly to the host user. We use unprivileged LXC containers and punch a specific hole in the ID map. Container user "dave" maps to host UID e.g. 1000.
...
* **Noise Reduction:** A full operating system generates massive background noise. We must filter this. Filtering by UID fails because dave has passwordless `sudo` rights. Escalated processes or detached cron jobs would escape the filter. We discard UID tracking. We filter eBPF events strictly by directory path and cgroup ID. 
...
1.  Configure `/etc/subuid` and `/etc/subgid` to allow the root user to delegate host UID/GID e.g. 1000.
...
1.  Create an Incus profile (`capsule-profile.yaml`).
2.  Define the raw ID mapping for UID/GID e.g. 1000 within the profile.
3.  Write the `cloud-init` configuration within the profile to create user dave, grant `sudo` access, install system dependencies, and install the required Python stack (`uv`, `ipython`, `playwright`, `numpy`, `pandas`, `matplotlib`).

### Phase 3: Runtime Execution
1.  Launch the Incus container using the Debian 13 Trixie image and the custom profile.
...
* **UID Mapping Check:** Create a file inside `/home/dave/host` from within the container. Verify from the host terminal that the file is owned by UID e.g. 1000, not root.
...
* **Observability Check:** Start the `bpftrace` monitor on the host. Trigger a background system update (`apt update`) inside the container. The log must remain silent. Read a file in `/home/dave/read-only/data`. The log must instantly output the JSON event.
* **Escalation Check:** Inside the container, run `sudo touch /home/dave/host/test.txt`. Verify the eBPF monitor on the host still captures the event despite the user switching to root.
